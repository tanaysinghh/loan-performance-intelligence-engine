...
- **The scenario engine's credit channel is not identified.** Macro levels are constant across loans within a month, so with one realised macro path a loan-level model cannot separate unemployment from calendar time. The symptom is diagnostic: a 2.3pp unemployment shock moves projected 12-month default by 0.42% in relative terms, and the high-prepayment scenario *raises* projected default. **Use Engine B (macro-conditioned Markov) to size credit stress; use Engine A only for which-loans segment detail.** Engine B moves cumulative 12-month default from 0.173 to 0.180 under adverse conditions.
